export const CONSTANTS = {
  SPLASH_SCREEN: 'SPLASH_SCREEN',
  TOKEN: 'TOKEN',

  GET_EVENTBRITE_CONFIG_REQUESTED: 'GET_EVENTBRITE_CONFIG_REQUESTED',
  GET_EVENTBRITE_CONFIG_SUCCEEDED: 'GET_EVENTBRITE_CONFIG_SUCCEEDED',
  GET_EVENTBRITE_CONFIG_FAILED: 'GET_EVENTBRITE_CONFIG_FAILED',

  DEEP_LINK_REQUESTED: 'DEEP_LINK_REQUESTED',
  DEEP_LINK_SUCCEEDED: 'DEEP_LINK_SUCCEEDED',
  DEEP_LINK_FAILED: 'DEEP_LINK_FAILED',

  SIGNUP_REQUESTED: 'SIGNUP_REQUESTED',
  SIGNUP_SUCCEEDED: 'SIGNUP_SUCCEEDED',
  SIGNUP_FAILED: 'SIGNUP_FAILED',

  SIGNIN_REQUESTED: 'SIGNIN_REQUESTED',
  SIGNIN_SUCCEEDED: 'SIGNIN_SUCCEEDED',
  SIGNIN_FAILED: 'SIGNIN_FAILED',

  SOCIAL_LOGIN_REQUESTED: "SOCIAL_LOGIN_REQUESTED",
  SOCIAL_LOGIN_SUCCEEDED: "SOCIAL_LOGIN_SUCCEEDED",
  SOCIAL_LOGIN_FAILED: "SOCIAL_LOGIN_FAILED",

  SIGNOUT_REQUESTED: 'SIGNOUT_REQUESTED',
  SIGNOUT_SUCCEEDED: 'SIGNOUT_SUCCEEDED',
  SIGNOUT_FAILED: 'SIGNOUT_FAILED',

  RESET_PASSWORD_REQUESTED: 'RESET_PASSWORD_REQUESTED',
  RESET_PASSWORD_SUCCEEDED: 'RESET_PASSWORD_SUCCEEDED',
  RESET_PASSWORD_FAILED: 'RESET_PASSWORD_FAILED',

  FORGOT_REQUESTED: 'FORGOT_REQUESTED',
  FORGOT_SUCCEEDED: 'FORGOT_SUCCEEDED',
  FORGOT_FAILED: 'FORGOT_FAILED',

  RESEND_EMAIL_REQUESTED: 'RESEND_EMAIL_REQUESTED',
  RESEND_EMAIL_SUCCEEDED: 'RESEND_EMAIL_SUCCEEDED',
  RESEND_EMAIL_FAILED: 'RESEND_EMAIL_FAILED',

  CLEAR_ERROR: 'CLEAR_ERROR',
  CLEAR_DEEP_LINK_ERROR: 'CLEAR_DEEP_LINK_ERROR',

  LOGOUT: 'LOGOUT',
};
